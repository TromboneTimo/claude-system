...
## Rule 5: On reported visual bugs, enumerate all axes before writing a fix (2026-04-22)

**Triggering incident:** Samurai Brass album carousel, Otto Cristofoli site. User said "remove white background from album covers." I shipped three iterations because I kept addressing only the axis I noticed in the latest screenshot instead of enumerating all possible causes up front.

- Pass 1: threshold=240 guess. Missed pixels at 248 AND missed the entire grey gradient from 122-205 (CD jewel-case highlight).
- Pass 2: per-album bg-color padding fixed the letterbox. Did not fix case edges.
- Pass 3: stricter threshold + inward inset. Finally clean.
- Parallel bug: never toggled the JA language button after editing the component. SIDE A/SIDE B were hardcoded English, track titles were always `{t.title}` (English) with JP in parens. Would have been caught in 30 seconds.

**Why:** I shipped based on what I assumed was true instead of verifying. Each "fix" addressed one visible symptom; the other symptoms were invisible until the next screenshot arrived.

**How to apply:**
- For any reported visual issue, list all plausible axes before writing code: source asset, data/config, CSS container, component logic, animation, language modes, responsive breakpoints.
- Image trim/crop: always sample edge RGB values first. Never guess thresholds.
- When padding trimmed images to fit a container, use the EXACT container bg color from the data file (per-item if it varies). Never default to black.
- Lang-aware component edits: grep the file for hardcoded English, and click the language toggle in the browser before declaring done.
- Three iterations on the same complaint = the original enumeration was incomplete. Stop, re-enumerate, do not re-guess.

Canonical protocol: `~/.claude/knowledge/visual-self-qa-protocol.md` (see "Diagnosing Reported Visual Bugs" section).
