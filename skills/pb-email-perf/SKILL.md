...
- Manual: Timo says "/pb-email-perf", "what's working on emails", "email perf brief", "show me what performs".
...
2. Bash-curl the endpoint:
   ...
   curl -sS -H "x-perf-secret: $SECRET" \
     ...
     > /tmp/pb-email-perf.json
   ```
3. Read `/tmp/pb-email-perf.json` and pretty-print the brief in chat using the format below.
...
- Not a substitute for reading the voice catalog or the FINAL emails. Performance data tells you WHAT worked, not HOW Harrison's voice works. Both are required for pb-email.

## Maintenance

- The endpoint code is at `api/ac-perf.js` in the project repo.
- The URL classifier (in ac-perf.js, `classifyUrl()`) needs updating whenever Harrison adds a new destination type. Currently buckets: youtube, masterclass, strategy-session, resource, zoom_live, social, hirose, reply, other.
