---
name: va-email
description: Email proposal engine for Victor Alegria Music business. Mirrors pb-script architecture but produces ready-to-send DRAFT emails (not video scripts). 6 parallel mining agents read sales calls, testimonials, winning ads, and Victor's existing email sequence to surface 5 fresh email drafts. Each draft includes subject line + 2 alternates, preheader, full body in Victor's voice (with the recurring tagline verbatim), P.S. matching one of 5 proven types, CTA from Section 5 templates, audience tag (broadcast / re-engagement / webinar-push / discovery-followup), hook angle, pain point, rationale, and VOC quote evidence. A 7th sequential auditor enforces voice fidelity, freshness across runs, and 1+ testimonial + 1+ sales-call quote per draft. Use when Timo says "email ideas", "email proposals", "draft emails", "what email should we send", "/va-email", or after AC sends are logged and we need fresh angles. Stops at the 5-draft menu and waits for Timo to pick. Do NOT auto-push to dashboard; pb-email-push handles that.
---
...
Speed is never the priority for this skill. Voice fidelity is. Victor's email list is small, focused, and high-signal. One off-voice email kills trust faster than ten on-voice ones build it. If a draft does not sound like Victor wrote it, throw it away and start over. Do not push borderline drafts and "iterate later".

This principle is also stored in `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_ship_right_not_fast.md`.

## What this does

Mirrors the pb-script architecture compressed into a single phase: 6 parallel specialist subagents mine Victor's VOC corpus through different lenses, each returns a complete DRAFT email (not just an idea), then a sequential auditor selects 5 final drafts enforcing voice and source diversity. The menu item IS the draft. Timo picks; pb-email-push lands them in `dashboard/emails.html` for Victor's review.
...
1. `voc/emails/extracts/victor-email-voice.md` (the 302-line voice catalog)
...
NEVER load emails 8-12 of the webinar sequence. They are signed by Paul The Trombonist and target music educators. Per `voc/emails/extracts/victor-email-voice.md` Section 10, they are template leakage and will pollute Victor's voice.
...
| 4 | Winning-pattern | `voc/emails/raw/winning-emails/` (currently empty) and `voc/meta-ads/*` winners | 1 draft mirroring proven hook/structure |
...
- Preheader
- Full body (Victor voice, %FIRSTNAME% merge tag, recurring tagline verbatim)
...
> Pick which to push to Victor's dashboard. Reply "push 1, 3, 5" or "push 2" or "all of them". I'll run pb-email-push.
...
1. `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/MEMORY.md`
2. `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_master_lessons.md`
3. `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_victor_voice.md`
4. `voc/emails/extracts/victor-email-voice.md`
...
- Does NOT modify `voc/emails/extracts/victor-email-voice.md`. The catalog is the source of truth.
...
1. **Off-voice drafts.** If you can't find real voice in Victor's corpus, don't fabricate it. Pull a different sales call or testimonial.
