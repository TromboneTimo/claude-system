...
3. **Live AC perf brief**. call `pb-email-perf` (or read `/tmp/va-email-perf.json` if pb-email already loaded it) for top 10 by open rate / CTR / reply rate, destination mix, subject patterns.

4. **Voice catalog + 3 FINAL emails + sequence emails 1-7**. load same as Agents 1-6 so the draft sounds like Harrison.
...
4. **Skipping the AC perf brief**. if the brief exists at `/tmp/va-email-perf.json`, you MUST read it. Drafting blind to live data is the original sin this agent was created to fix.
