---
name: va-email-push
description: Phase 1.5 of Victor Alegria Music email engine. Pushes Timo-approved email drafts from the pb-email chat conversation into the Supabase-backed dashboard so Victor can review them at dashboard/emails.html. Takes the drafts Timo greenlit (e.g. "push 2 and 4 to the dashboard", "send these to victor", "upload the dental-trigger one") and inserts them into the email_proposals table with status=proposal_pending. Each proposal includes subject + 2 alts, preheader, full body, P.S. text and type, hook angle, audience, CTA, rationale, VOC quotes, and source tags. After Victor approves on the dashboard, the email is ready to send via ActiveCampaign (publish to AC is Phase 2). Use this skill any time Timo says "push to dashboard", "send to victor", "upload these emails", "queue these for victor", "/va-email-push", or after a pb-email run when Timo picks 1+ drafts to send for review.
---
...
# pb-email-push. Phase 1.5: Push email drafts to Victor's dashboard.
...
Stored in `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_ship_right_not_fast.md`.
...
Victor approves on the dashboard. Email moves to `approved`. Then Timo (or the future AC publish step) sends it via ActiveCampaign.

## When to fire

Trigger when Timo says (after a `/va-email` run):
- "push draft 2 to the dashboard"
- "send drafts 1 and 3 to victor"
- "upload these emails"
- "queue these for victor's review"
- "/va-email-push"
- "ship the dental-trigger one"

Do NOT fire if Timo has not yet seen drafts in chat. Run `/va-email` first.
...
> "Pushing these 2 drafts to Victor's dashboard:
> 1. {subject 1}
> 2. {subject 2}
...
Read `/Users/air/Desktop/Victor Alegria Music/dashboard/lib/config.js`. If the file does not exist or contains placeholder `YOUR-PROJECT-REF`, STOP and tell Timo to run `dashboard/setup/SUPABASE_SETUP.md` first.
...
python3 build_email_payloads.py  # writes /tmp/va-emails/{0,1,2}.json

for f in /tmp/va-emails/*.json; do
...
> Victor can review them at: dashboard/emails.html (Pending Review column).
> Want me to open it?"

If Timo says yes, run `open -a Safari /Users/air/Desktop/Victor Alegria Music/dashboard/emails.html`.
...
Add a one-liner:
> "Pushed N email drafts to Victor dashboard: [subjects]. Awaiting approval."
...
4. **Skipping the approval wait.** Do NOT auto-fire any AC publish step after pushing. Victor has to approve first on the dashboard.
...
- Does not approve drafts on Victor's behalf (Victor clicks the button).
