---
name: va-script
description: Phase 1 of Victor Alegria Music YouTube content engine. Research-driven idea generator that spawns 6 parallel specialist subagents (raw deep-dive, hidden pain hunter, objection dissolver, verified winner pattern, conversion trigger detector, fresh lens) plus a sequential voice-diversity auditor to read across the full voice-of-customer corpus and return 5 conversion-trigger-driven content ideas with verbatim quote evidence and FRESH voices each run. 1-2 ideas anchored on the proven verified-winner pattern (YouTube winners + FB winning ads), 3-4 ideas surfaced from raw transcripts, hidden pain framework, objection lenses, and rotating fresh angles. Voice diversity log prevents recycling the same speakers across runs. Use this skill whenever Timo or Victor asks for content ideas, video ideas, what to film next, what Victor should make, "give me angles", "youtube ideas for victor", "next video for victor", "precision brass content ideas", "/va-script", or any variation. Also fire when the user mentions Victor and content/video/script in the same breath. Do NOT generate the script itself. That is Phase 2 and lives in a future companion skill. This skill stops at the 5-idea menu and waits for Timo to pick one.
---
...
Generate 5 YouTube content ideas for Victor Alegria that are anchored in **what verifiably caused his prospects to buy**. Not generic trumpet advice. Not what's trending. Ideas backed by the actual language, fears, identity arcs, and conversion triggers found in Victor Alegria Music's voice-of-customer corpus.
...
- **1-2 ideas anchored on VERIFIED WINNERS** (Agent 4), pattern-matched against `voc/youtube/raw/` (status=winner videos) AND `voc/meta-ads/` (status=winner ads). Both are proven-conversion sources.
...
Prompt:
```
You are doing a deep-dive read of ONE raw corpus from Victor Alegria Music's voice-of-customer database. Your corpus this run: [CORPUS_ID].

Funnel layer for this run: [TOFU/MOFU/BOFU].

Read `~/.claude/skills/va-script/references/raw-deep-dive-rotation.md` first to see the per-corpus reading instructions. Then read EVERY file in your assigned corpus end-to-end. Do NOT read summary files (voice-bank.md, won-deals-voice-bank.md, etc.). Those are concentrated and you are mining fresh.

Your job:
1. Identify named speakers in your corpus. Note WHICH speakers appear (this feeds the voice-diversity log).
2. Extract 5-10 verbatim quotes from speakers not over-used in recent runs (recycled-voice list is built from voices_used_log.jsonl; no pre-seeded list for Victor). Prefer fresh voices.
...
Prompt:
```
You are hunting for HIDDEN PAIN in voice-of-customer corpus. Hidden pain is a wound prospects circle around in sales calls without ever naming directly. Naming the unnamed pain is the highest-converting hook pattern.

Funnel layer for this run: [TOFU/MOFU/BOFU].

Read `~/.claude/skills/va-script/references/hidden-pain-framework.md` first for the 8 hidden problems (HP1-HP8) and detection patterns (compensation clauses, hypothetical distancing, excessive technical detail, apologizing for the goal, repeated reassurance-seeking).

Then read these files for traces of subtext:
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/raw/ (sample 6-8 calls, prefer speakers not over-used in recent runs per voices_used_log.jsonl)
- /Users/air/Desktop/Victor Alegria Music/voc/synthesis/2026-04-21_deep-psychological-dive_victor-ball_19-prospects.md
- /Users/air/Desktop/Victor Alegria Music/context/prospect-psychology.md

Your job:
1. Pick 1-2 hidden problems (HP1-HP8) that are MOST under-addressed in `voc/youtube/raw/index.json` topic_tags.
...
Prompt:
```
You are pre-empting most common sales-call objections. Pick ONE objection lens and propose a video that dismantles it BEFORE the prospect ever gets on a call.

Funnel layer for this run: [TOFU/MOFU/BOFU].

Read `~/.claude/skills/va-script/references/objection-lenses.md` first for the 8 lenses (OBJ1-OBJ8) and the pre-emption pattern (HOOK / VALIDATION / DISMANTLE / DEMONSTRATION / RESOLUTION).

Then read these files:
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/raw/ (sample 6-8 calls, search for HESITATION/PUSHBACK moments specifically)
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/extracts/lost-deals-voice-bank.md
- /Users/air/Desktop/Victor Alegria Music/voc/synthesis/objection-library.md
- Facebook ad comments where present (`voc/meta-ads/*/comments-top.md`)
...
4. Specify the DISMANTLE LINE (the specific reframe that flips the objection) and the DEMONSTRATION (what Victor shows on camera to falsify the objection).
...
Prompt:
```
You are researching what verifiably converts for Victor Alegria Music business across two channels: YouTube long-form and Facebook ads.

Funnel layer for this run: [TOFU/MOFU/BOFU].

PART 1. YouTube winners
READ EVERY VIDEO marked status=winner in /Users/air/Desktop/Victor Alegria Music/voc/youtube/index.json. Raw files live in voc/youtube/raw/<vid>/ (transcript.md, comments.json, metadata.json); derived files live in voc/youtube/extracts/<vid>/ (analysis.md, comments-top.md). If empty, note and continue.

PART 2. Facebook winning ads
READ EVERY AD marked status=winner in /Users/air/Desktop/Victor Alegria Music/voc/meta-ads/index.json. Raw files live in voc/meta-ads/raw/<ad>/ (creative/copy.md, performance.json, metadata.json); the analysis lives in voc/meta-ads/extracts/<ad>/analysis.md. If empty, note and continue.

PART 3. Reference materials (always read)
- /Users/air/Desktop/Victor Alegria Music/references/converting-video-embouchure-transcript.md
- /Users/air/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/project_proven_converter_template.md (12-move converter template)
...
Prompt:
```
You are detecting CONVERSION TRIGGERS in customer corpus. A conversion trigger is the exact line, realization, or demonstration that flipped a prospect into a buyer.

Funnel layer for this run: [TOFU/MOFU/BOFU].

Read RAW testimonial files end-to-end (these are CUSTOMERS who completed the program, not prospects):
- /Users/air/Desktop/Victor Alegria Music/voc/testimonials/raw/ (all 11 files)

Then for additional signal:
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/extracts/won-deals-quotes.jsonl
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/extracts/sales-call-outcomes.jsonl

Reference the 8-trigger taxonomy in `~/.claude/skills/va-script/references/conversion-triggers.md` (T1-T8).
...
Prompt:
```
You are researching the [LENS_NAME] angle for content. This lens was picked because it has NOT been used in the last 3 runs (per voices_used_log.jsonl).

Funnel layer for this run: [TOFU/MOFU/BOFU].

The 12 PLACEHOLDER lenses (audition-prep, refine after mining): audition-odds-anxiety, unstructured-practice, peaking-and-taper, sound-concept-panel, mindset-self-belief, accountability-commitment, recording-method, talent-vs-preparation, pro-track-identity, deadline-pressure, teacher-loyalty, comparison-to-top-players.

Read across these files for traces of [LENS_NAME]:
- /Users/air/Desktop/Victor Alegria Music/voc/synthesis/all-quotes.jsonl
- /Users/air/Desktop/Victor Alegria Music/voc/sales-calls/raw/ (sample 6-8 random calls)
- /Users/air/Desktop/Victor Alegria Music/voc/emails/raw/sequences/2026-04-21_email-sequence_webinar-optin-to-strategy-session.md
- /Users/air/Desktop/Victor Alegria Music/voc/emails/extracts/victor-email-voice.md
- /Users/air/Desktop/Victor Alegria Music/context/victor-profile.md

Find the most NON-OBVIOUS angle on this lens. Not what Victor already says publicly. Something the data shows is loaded with emotion but Victor hasn't directly addressed in a video yet.

Propose 1 wildcard video idea through this lens. Must:
- Pass ICP checklist
- Cite 3+ verbatim quotes with source files
- Be something NOT covered in `voc/youtube/raw/index.json` topic_tags
- Sell identity, not technique
- Use a fresh primary voice (not over-used per voices_used_log.jsonl)
...
3. **Quote-sourcing minimums**: 1+ testimonial quote AND 1+ sales call quote per idea (per `feedback_quote_sourcing_minimums.md`). Victor-quotes need a `conversion_lens` field.
...
After locking the final 5, append one line to `/Users/air/Desktop/Victor Alegria Music/voc/voices_used_log.jsonl` with run_id, corpus_picked, idea_ids, primary_voices, secondary_voices, raw_files, lenses (see `references/voice-diversity-protocol.md` for exact format).
...
4. **Hard-block generic ideas.** If an idea would also make sense for a Berklee jazz student or a high-school marching band kid, it's too generic for Victor's ICP. Reject and replace.
...
7. **Never let Victor film without script approval.** When presenting the menu, end with the standard reminder: "Once you pick one, I'll generate the full script and visual plan. Send to Timo for review before filming."
...
- Not for non-Victor content (Robinson's Remedies, Creator Conservatory, etc. have their own skills)
