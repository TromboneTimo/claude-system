...
29. NEVER declare a slide viewport-safe based on a 1920x1080 headless render. Safari's chrome (tab bar + URL bar + bookmarks) eats 100-200px. A slide that passes at 1080 height can clip eyebrow/top content in a real Safari window at 680-900px. REQUIRED: render at 1440x680 (minimum realistic Safari viewport). If it passes there, it passes everywhere. Also check 760 and 820 for hero-heavy or image-heavy slides. Bash loop: `for H in 680 760 820; do chrome --headless --screenshot=/tmp/s-h${H}.png --window-size=1440,${H} ...; done` then Read each. Design budget: ~600px total content at 680px window. Flexbox `align-items: center; justify-content: center` pushes overflow OUT BOTH WAYS — top eyebrow gets clipped as often as bottom kicker. Cuts that buy vertical space (priority order): (a) remove decorative image ~200px, (b) shrink headline clamp max by 10-20%, (c) tighten paragraph line-height from 1.5 to 1.4, (d) reduce card padding.
30. NEVER declare a multi-slide deck done after capturing only slide 1. Decks using `.active` class toggles, scroll-snap, or `height:100vh` sections will freeze at slide 1 in default headless capture. To verify slide N: (a) for `.active` systems, copy HTML to /tmp and `sed` the `active` class onto the target `<section>`; (b) for scroll-snap systems, `sed` `scroll-snap-type: y mandatory` → `none` AND `height: 100vh` → `height: auto; min-height: 1080px`, then tall-viewport capture (≤5000px, 20000px hangs). Then Read the PNG for each slide you edited. No screenshot = no QA.
31. NEVER ship deck copy without running the em dash sweep. Run `grep -n "&mdash;\| — "` across every file edited. Only permitted hits: HTML comments (`<!--`), `<title>`, `alt=`, and name attribution lines (`— Sarah K.`). Sweep pre-existing copy too — em dashes from prior sessions are still violations. Replacements: ` — ` → `. `, `&mdash;` → `.` or `, ` or `/` depending on grammatical role.
32. NEVER generalize a proper noun the user named in their prompt. If they said "Claude Code," the slide says "Claude Code" — not "AI tools" or "tools your agency hasn't heard of." Generalizing a named specific into an abstract category kills the thesis and reads as safe-fabrication. Decision rule: scan your generated copy for every proper noun in the user's most recent message — each one must appear verbatim in the relevant slide, or the slide isn't done.
