---
name: va-ideas-push
description: Phase 1.5 of Victor Alegria Music YouTube content engine. Pushes Timo-approved video ideas from the pb-script chat conversation into the Supabase-backed dashboard so Victor can review them at dashboard/scripts.html. Takes the ideas Timo greenlit (e.g. "push idea 2 and 4 to the dashboard", "send these to victor", "upload the chops idea") and inserts them into the ideas table with status=idea_pending. Each idea includes title, pain point, hook angle, 1-paragraph rationale, VOC quote evidence, and source tags. After Victor greenlights an idea on the dashboard, Timo runs pb-script-write on the approved idea ID. Use this skill any time Timo says "push to dashboard", "send to victor", "upload these ideas", "queue these for victor", "/va-ideas-push", or after a pb-script run when Timo picks 1+ ideas to send for review.
---
...
This principle is also stored in `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_ship_right_not_fast.md` and loads on every session.

## What this does

Bridges `pb-script` (Phase 1, brainstorm in chat) and `pb-script-write` (Phase 2, full filming script). Pushes Timo-approved ideas into the Supabase `ideas` table so Victor sees them on `Precision-Brass/dashboard/scripts.html` in the "Ideas Pending" column.

Victor greenlights → idea moves to "Ideas Approved" → Timo runs `pb-script-write` → full script lands in "Script Pending".

## When to fire

Trigger when Timo says (after a `pb-script` run):
- "push idea 2 to the dashboard"
- "send ideas 1 and 3 to victor"
- "upload these"
- "queue these for victor's review"
- "/va-ideas-push"
...
### Mode A: Casual chat ideas (post-`/va-script` brainstorm)
...
The proposal has these labelled sections: Layer, Idea origin, Concept (with Rough flow), The wound we're naming, Why this will convert, Source evidence (with attributed quotes), ICP segment target, How this video connects to Victor's existing channel.
...
2. **Enumerate**. List every labeled section heading in his proposal text. Examples: Layer, Idea origin, Concept, Rough flow, The wound we're naming, Why this will convert, Source evidence, What these quotes show together, ICP segment target, How this video connects to Victor's existing channel.
...
<div class="r-section channel">
  ...
  <p>{How this connects to Victor's existing channel.}</p>
</div>
...
- Testimonial quotes: `"{Name}. Video testimonial after working with Victor, describing {context}."`
...
1. **One quote from a testimonial** (someone who worked with Victor and recorded a video testimonial). Source: `voc/testimonials/raw/`.
2. **One quote from a sales call** (someone Victor did a discovery call with). Source: `voc/sales-calls/raw/`.
...
Skip this rule and the proposals start to feel repetitive to Victor. Fresh voices = stronger evidence per proposal.

## Victor-quote conversion-lens rule (added 2026-04-26)

If a quote in the array has Victor as the speaker (his own ad copy, his own video transcript, his own social post), the `source` field must do two things:
...
Without the conversion lens, Victor-quotes are self-citation. Bad example: `"own Facebook ad copy. Hook line of currently-performing ads."` Good example: `"own Facebook ad copy. The reason this exact framing converts on cold traffic: it gives comeback players permission to stop blaming their own body for damage a teacher caused. That permission is the prerequisite for being open to a new program."`

See `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/feedback_quote_sourcing_minimums.md`.
...
> "Pushing these 2 ideas to Victor's dashboard:
> 1. {title 1}
> 2. {title 2}
...
Read `/Users/air/Desktop/Victor Alegria Music/dashboard/lib/config.js`. If the file does not exist or still contains the placeholder `YOUR-PROJECT-REF`, STOP and tell Timo:
...
```bash
SUPABASE_URL="$(grep SUPABASE_URL /Users/air/Desktop/Victor Alegria Music/dashboard/lib/config.js | cut -d\' -f2)"
SUPABASE_KEY="$(grep SUPABASE_ANON_KEY /Users/air/Desktop/Victor Alegria Music/dashboard/lib/config.js | cut -d\' -f2)"
...
> Victor can review them at: dashboard/scripts.html (Ideas Pending column).
> Want me to open it?"

If Timo says yes, run `open -a Safari /Users/air/Desktop/Victor Alegria Music/dashboard/scripts.html`.

### Step 6. Log to PRIORITIES.md (silent)

Add a one-liner to `~/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/SESSION_LOG.md`:
> "Pushed N ideas to Victor dashboard: [titles]. Awaiting greenlight."
...
4. **Skipping the greenlight wait.** Do NOT auto-run `pb-script-write` after pushing. Victor has to greenlight first on the dashboard.

## What this skill does NOT do

- Does not generate ideas (that's `pb-script`)
- Does not write the full filming script (that's `pb-script-write`)
- Does not approve ideas on Victor's behalf (Victor clicks the button)
