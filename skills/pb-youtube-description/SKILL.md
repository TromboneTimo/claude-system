---
name: pb-youtube-description
description: Generates Harrison Ball's Precision Brass YouTube description + chapters from a finished video file. Transcribes the video with mlx-whisper, identifies beat boundaries with real timestamps (no guessing), writes curiosity-driven chapter titles (NOT generic labels like "Intro"), assembles the description using Harrison's locked template (Free Masterclass link line 1, Testimonials playlist line 2, 3-paragraph hook, Chapters block, hashtags), and presents it as plain markdown in chat for Timo to approve BEFORE any file write. Use this skill whenever Timo says "make a YouTube description", "write the description for this video", "youtube description for X", "description and chapters", "/pb-youtube-description", "describe this video for YouTube", or pastes a video file path (.mov / .mp4) and asks for the description. ALWAYS load the locked template at `references/description-template.md` before drafting. Output plain markdown in chat first (chat-draft-before-render rule). Never write to files until Timo approves.
---

# pb-youtube-description. YouTube description + chapters for Harrison's videos
...
3. **Read the project memory `canon_working_process.md`.** This skill is a deliverable-producing skill. Show draft in chat FIRST. No file writes until Timo approves.
...
Read the SRT and mark each topic shift. Harrison's videos typically follow:
...
- Why Harrison built [system / VAS / converter]
...
- Use Harrison-voice fragments: "The truth:", "The lie:", "Why I built", "Try this at home", "The correct order"
...
Notes on the literal byte format Harrison uses:
- Masterclass URL: `https://www.precisionbrass.info/webinar-registration-pb`. YouTube will truncate it visually in the description preview (renders as `webin...`); paste the full URL.
...
This skill produces a description for Harrison/Timo to paste into YouTube manually. There is no auto-upload.
...
5. **Writing the description from the script instead of the actual filmed video.** Harrison often changes wording on-set. Always transcribe the FINAL filmed file, not the pre-shoot script. If only the script is available, ASK before proceeding.
...
- Harrison changes the standing CTA URL or testimonials playlist
- A new hashtag enters Harrison's rotation
