...
- Manual: any time Timo says "write the script", "make the script", "/va-script-write", "phase 2", "draft the script for X"
...
4. `/Users/air/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/canon_script_writing.md`. Project-level failure memory.
5. `/Users/air/.claude/projects/-Users-air-Desktop-Victor-Alegria-Music/memory/canon_script_writing.md`. **The 7 master lessons from 2026-05-06. Read these or repeat the 15-iteration mistake.**
6. `/Users/air/Desktop/Victor Alegria Music/references/converting-video-embouchure-transcript.md`. Source-of-truth Victor voice.
...
- `voc/synthesis/` (deep psychological dive on 19 prospects.)
...
- `voc/synthesis/*.jsonl` (pre-tagged: all-quotes, comments-quotes, lost-deals, won-deals, sales-call-outcomes.)
- `voc/synthesis/*.md` (voice banks: comments, lost-deals, won-deals, victor-email-voice, voice-bank, objection-library.)
- `voc/youtube/raw/` (per-video folders with transcripts, top 200 comments, sales attribution.)
- `voc/meta-ads/` (per-ad folders with copy, performance, sales attribution.)
...
2. **Testimonials before-state:** all 11 testimonials. What they were doing wrong BEFORE Victor fixed them.
...
6. **Teacher-blame + bad-advice + YouTube/FB engagement:** sales calls + research + voc/youtube/raw comments + voc/meta-ads winning-ad comments. Specific bad advice prospects got. Specific reactions to Victor's existing content.
...
## Instructions are directive TO Victor, not abstract director-speak (2026-05-06)

Write all bullets and director notes as second-person instructions Victor can act on. NOT third-person abstract intent.

- BAD: "Victor transitions from naming the problem to proving it on camera."
...
Trim every "Victor X" to a verb the reader can do. Trim every "the viewer" reference unless the action genuinely requires the audience POV.

Confirmed 2026-05-06: Timo flagged the v2 script for being too descriptive ("Victor transitions...") instead of directive ("Cut to camera. Hit the next beat."). Rule applies to action blocks, bullets, bridges, and section labels.
...
## Bullet anchors are intent, not text Victor reads (2026-05-06)

**Riff bullets must be 1 line of director's intent. Never write the sentences Victor would say on camera.**
...
- Locked one-liners (verbatim quotes Victor must say word-for-word) stay long if needed. Only exception.
- Metaphors and reframes get a short LABEL (e.g. "drop the marathon-with-a-weight metaphor"), not the full speech.

**Metaphor exception (CRITICAL): if a bullet references a metaphor by short label, the metaphor itself MUST be defined inline the first time it appears in the doc.** Victor does not have your context. He cannot expand a label he hasn't seen the full version of. Pattern: in the FIRST step that uses the metaphor, write one sentence spelling out what the metaphor is, then in later steps you can use the short label. If the metaphor is in the script header (Backbone metaphor: X), still spell it out in the body the first time it lands.

When in doubt, cut. Victor improvises better than any draft can write for him. Long bullets kill his voice. Confirmed twice on the 2026-05-06 famous-method-killing-embouchure script. See `canon_script_writing.md`.

## Voice rules

Match Victor's converter fingerprint. Examples in `references/example-mouthpieces-script.md`:
...
1. `/Users/air/Desktop/Victor Alegria Music/scripts/YYYY-MM-DD_<slug>.{html,pdf}` (CANONICAL ARCHIVE. both HTML and PDF.)
2. `/Users/air/Desktop/Victor Alegria Music/output/YYYY-MM-DD_<slug>.{html,pdf}` (working copy. both HTML and PDF.)
...
- **Riff topics** (regular weight, director's notes): trap explanations, mistake elaborations, test riffs. Victor improvises off these.
- **Visual specs**: preserve EXACTLY as Timo writes them. If he says "Victor sitting in front of all his mouthpieces", that detail is mandatory. Don't compress it.
- **[HARRISON FILLS]**: tag specific gear / tunes / student names that need Victor's input. Do not invent.
...
6. NO meta-instructions inside the deliverable ("send to Timo", "Victor fills" lists go in conversation, not on the page).
...
10. NO manhandling Victor's improv. Locked lines for critical moments, riff topics for the rest.
...
1. **Detect dashboard mode.** If invoked with `/va-script-write i_YYYYMMDD_slug`, this is a dashboard-driven run. Fetch the originating idea from Supabase first:
   ```bash
   ...
   ```
   Use the idea's title, pain_point, hook_angle, voc_quotes, source_tags as the conversion-trigger context. If status is not `idea_approved`, STOP and tell Timo (e.g. already scripted, or Victor hasn't greenlit yet).
...
5. Draft each beat in Victor's voice with the locked-lines / bullet-riff split.
...
8. **Second checkpoint in chat (do NOT upload yet).** Show Timo:
   ...
   - Explicit prompt: **"Open the PDF at {path}. Reply 'go' to upload to Victor's dashboard, or tell me what to fix."**
9. **Upload to Victor's dashboard (only AFTER Timo's go).** Build a script payload with the body filled in, POST to Supabase, then mark idea as scripted. See "Dashboard upload" below.
...
Two uploads happen: (1) the PDF goes to Supabase Storage so Victor can download it, (2) the rich HTML body + PDF URL go into the script row so the dashboard renders the script in-modal AND shows a Download PDF button.

### 9a. Upload the PDF to Supabase Storage

```bash
source ~/.claude/secrets/precision-brass.env
PDF_PATH="/Users/air/Desktop/Victor Alegria Music/scripts/{filename}.pdf"
...
The `body` field is a JSON array with **a single entry containing the FULL HTML DOCUMENT (head + style + body) AND the PDF URL**. The dashboard renders this content inside an iframe via `srcdoc`, so the `<style>` block in the `<head>` is REQUIRED. If you upload only the body innerHTML (without the head/style), the iframe gets unstyled HTML and Victor sees a wall of plain text instead of the colored layout.
...
- **Body MUST contain real content.** Never push an empty/stub body. Victor opening an empty script kills trust. If you can't fill the 7 beats, do not push.
- **Always do step 9b (mark idea scripted).** Otherwise the idea stays in Timo's "Ideas Approved" and confuses the kanban.
- **If the idea is not in `idea_approved`, STOP.** Don't override Victor's state.
